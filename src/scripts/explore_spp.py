...
def plot_all_fields_at_step(path: Path, step: int = 0) -> None:
    ...
    for i, field in enumerate(scalar_fields):
        ...
        plotter.add_mesh(pvmesh.copy(), scalars=field, show_edges=True)
    ...
    plotter.link_views()
    plotter.add_title(f"{path.stem}  all scalar fields  t={t:.4g}", font_size=8)
    plotter.show()
...
def plot_p1_vs_p6(field: str, step: int = 0) -> None:
    ...
    pvmesh_p1 = load_mesh(LOBULE_P1, step=step)
    pvmesh_p6 = load_mesh(LOBULE_P6, step=step)

    plotter = pv.Plotter(shape=(1, 2))
    plotter.subplot(0, 0)
    plotter.add_mesh(pvmesh_p1, scalars=field, show_edges=True)
    plotter.add_title(f"p1  {field}  t={meta_p1['times'][step]:.4g}", font_size=8)
    plotter.subplot(0, 1)
    plotter.add_mesh(pvmesh_p6, scalars=field, show_edges=True)
...
def plot_deformation_vectors(field: str = "u", step: int = 0) -> None:
    ...
    pvmesh = load_mesh(DEFORMATION, step=step)
    t = meta["times"][step]

    if field not in pvmesh.point_data:
        print(f"Field '{field}' not found in point data. Available: {list(pvmesh.point_data.keys())}")
        return

    plotter = pv.Plotter()
    plotter.add_mesh(pvmesh, show_edges=True, opacity=0.5, color="lightgray")
    glyphs = pvmesh.glyph(orient=field, scale=field, factor=0.01)
    plotter.add_mesh(glyphs, color="red")
    plotter.add_title(f"deformation  {field} glyphs  t={t:.4g}", font_size=9)
    plotter.show()
